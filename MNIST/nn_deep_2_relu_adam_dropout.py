...
# Added dropout to training with keeping probability of 0.75 (75%)
...
Y1d = tf.nn.dropout(Y1, keep_prob=pkeep)
Y2 = tf.nn.relu(tf.matmul(Y1d, W2)+b2)
Y2d = tf.nn.dropout(Y2, keep_prob=pkeep)
Y3_logits = tf.matmul(Y2d, W3)+b3
...
for batch_number in range(4000):
    ...
    training_data = {X: X_batch,
                     Y_: Y_batch,
                     learning_rate: getLearningRate(batch_number, 0.0005, 0.025),
                     pkeep: 1}
